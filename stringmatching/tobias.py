...
class TobiasKMP(Base):
    def search(self, pattern, text, all=False):
        start = 0
        self.border = self.__strong_border(pattern)
        if all:
            results = []
            limit = len(text) - len(pattern)
            stop = False
            while start <= limit and not stop:
                result = self.__kmp(pattern, text, start)
            ...
            return [ self.__kmp(pattern, text, start) ]

    def __kmp(self, pattern, text, start):
        i = start
        n = len(text)
        m = len(pattern)
        j = 0
    ...
    def __strong_border(self, pattern):
        j = 0
        t = -1
        m = len(pattern)
        bord = [0]*(m+1)
